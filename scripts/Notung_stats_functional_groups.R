...
##### LOAD FUNCTIONAL GROUP AND SELECT ######

group <- read_tsv("~/Desktop/SMC_project/Notung/functional_groups/vesicle_transport", col_names = F)
group <- group$X1
tree_stats <- tree_stats[group]

tree_stats <- tree_stats[!is.na(names(tree_stats))]
...
write_tsv(final, "~/Desktop/SMC_project/Notung/NOTUNG_stats_vesicle_transport.tsv")
